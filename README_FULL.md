...
Поиск возвращает список *кандидатов* — уникальных путей-хлебных крошек. Когда найден ровно один кандидат, возвращается Markdown-справка по нему. Если кандидатов несколько, нужно уточнить запрос или использовать путь кандидата как точное ключевое слово.
...
В секции **«MCP инструменты»** расположены тумблеры для каждого из 10 инструментов. При переключении тумблера кэш списка инструментов сбрасывается, и следующий запрос `tools/list` от MCP-клиента вернёт актуальный набор. Настройки сохраняются между сеансами автоматически.
